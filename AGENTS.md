...
Chart tools are reusable, interactive line charts with optional quartile bands. These tools require **no custom JavaScript files or inline HTML scripts**—they use HTMX for interactivity and Chart.js for rendering. All chart initialization happens server-side via the `LineChart` templ component.
...
✅ **No inline HTML scripts** – External libraries only (htmx, chart.js)  
✅ **Server-side chart generation** – Chart.js setup is generated by templ and swapped via HTMX

## 1. File Structure

Each chart tool requires three files:
...
The generic `LineChart` templ component (in `internal/templates/msindex.templ`) and `_chart-tools.scss` handle all chart rendering and styling. No custom JavaScript or SCSS files are needed.
...
✅ **CSP compliant** – No inline scripts or hardcoded colors

---

## 5. Download UI in Layout
...
4. **Not destroying old chart:** Chart.js destruction handled in templ via `window.lineChartInstance.destroy()`
